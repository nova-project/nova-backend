...
import net.getnova.backend.boot.logging.logback.converter.ColorConverter;
import net.getnova.backend.boot.logging.logback.converter.ExtendedWhitespaceThrowableProxyConverter;
...
public final class LogbackConfigurator implements LoggingConfigurator {

  private static final String PATTERN = "[%clr(%d{yyyy-MM-dd HH:mm:ss}){cyan}] [%clr(%5p)] [%clr(%15.15t){cyan}] "
    + "%clr(%-40.40logger{39}){cyan} : %clr(%m) %n${LOG_EXCEPTION_CONVERSION_WORD:-%wEx}";
  ...
  public LogbackConfigurator(final LogLevel level, final String dsn) {
  ...
  public void initialize(final LogLevel level, final String dsn) {
    ...
    this.addConversionRule("wEx", ExtendedWhitespaceThrowableProxyConverter.class);
