...
public final class RestApiLocation extends HttpLocation<FullHttpRequest> {
    ...
    protected void channelRead0(@NotNull final ChannelHandlerContext ctx, @NotNull final FullHttpRequest msg) throws Exception {
        ...
        final String content = msg.copy().content().toString(HttpUtil.getCharset(msg, CHARSET));
        final JsonObject json = JsonUtils.fromJson(JsonParser.parseString(content), JsonObject.class);

        final ApiResponse apiResponse = ApiExecutor.execute(this.endpoints, new ApiRequest(new URI(msg.uri()).getPath(), json, null));
